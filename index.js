...
function isOwner(jid) {
  ...
  return num === CONFIG.ownerJid.split("@")[0].split(":")[0] || num === CONFIG.ownerLid;
}
...
// ─── Sistema de licencias por token (por grupo) ───────────────────────────────
// db.tokens[code]     = { durationDays, maxUses, usedCount, createdAt, redemptions: [{group, redeemedBy, redeemedAt}] }
// db.groupLicenses[gid] = { expiresAt, lastCode, activatedBy, activatedAt }
function generateTokenCode(db) {
  db.tokens = db.tokens || {};
  let code;
  do {
    code = String(Math.floor(100000 + Math.random() * 900000)); // 6 dígitos, nunca empieza en 0
  } while (db.tokens[code]);
  return code;
}

function isGroupLicensed(db, groupJid) {
  const lic = db.groupLicenses?.[groupJid];
  return !!(lic && lic.expiresAt > Date.now());
}

// db.trialUsedGroups[groupId] = true. Se marca la PRIMERA vez que se usa #trial en
// un grupo y nunca se borra (ni con #reset, ni si el bot sale y lo vuelven a meter),
// así la prueba gratis de 2 días es de verdad una sola vez por grupo para siempre.
function hasUsedTrial(db, groupJid) {
  return !!db.trialUsedGroups?.[groupJid];
}
...
> (•ิ_•ิ) *Owner Only:*
 ...
 \`#bugs\` \`#delbug <id>\` \`#delbugs\` \`#rewardbug <id> [días]\` \`#clrcache\` \`#restart\`
 \`#gtoken <días> <usos>\` \`#tokens\` \`#token <código>\` \`#licencia\` \`#trial\` \`#licenseall\`
...
──
*Comando:* #rewardbug <id> [días=2]
*Descripción:* Premia el grupo de origen de un reporte/sugerencia con días extra de licencia (2 por defecto). Cada reporte solo se puede premiar una vez.
──
*Comando:* #clrcache
...
──
*Comando:* #gtoken <días> <usos>
*Descripción:* Genera un código de 6 dígitos que activa una licencia de <días> días por cada grupo que lo canjee, hasta agotar <usos> canjes (ej: #gtoken 30 5 = código de 30 días utilizable en hasta 5 grupos distintos). Exclusivo del owner original kashira.
──
*Comando:* #tokens
*Descripción:* Lista todos los tokens generados y cuántos usos les quedan.
──
*Comando:* #token <código>
*Descripción:* Canjea un token dentro del grupo que quieres activar (o extender, si ya tenía licencia). Cualquiera puede usarlo, no solo el owner.
──
*Comando:* #licencia
*Descripción:* Muestra si el grupo actual tiene una licencia activa y cuándo vence.
──
*Comando:* #trial
*Descripción:* Activa 2 días de prueba gratis para el grupo actual. Solo funciona una vez por grupo para siempre (aunque me saquen y me vuelvan a meter, no se puede repetir). Cualquiera puede usarlo, no solo el owner.
──
*Comando:* #licenseall dd/mm/yyyy [HH:mm]
*Descripción:* Activa licencia hasta esa fecha (hora opcional, default 12:00, zona horaria Ciudad de México) en TODOS los grupos donde esté el bot ahora mismo. Exclusivo del owner original kashira.
━━━━━━━━━━━━━━━━`,
...
async function handleCommand(sock, msg) {
  ...
  const aliasMap = {
    ...
    "rewardbug": "rewardbug", "premiarbug": "rewardbug",
    ...
    "gtoken": "gtoken", "generartoken": "gtoken",
    "token": "token", "canjear": "token", "activar": "token",
    "tokens": "tokens", "listtokens": "tokens",
    "licencia": "licencia", "license": "licencia",
    "trial": "trial", "prueba": "trial", "probar": "trial",
    "licenseall": "licenseall", "licenciatodos": "licenseall",
  };
  ...
  // ══════════════════════════════
  //    BLOQUEO: LICENCIA POR TOKEN (el grupo necesita un token activo)
  // ══════════════════════════════
  // Comandos siempre permitidos aunque el grupo no tenga licencia activa:
  // el propio sistema de tokens, el menú y la info del bot.
  const TOKEN_EXEMPT_CMDS = new Set(["token", "gtoken", "tokens", "licencia", "licenseall", "trial", "menu", "bi"]);
  if (isGroup && !TOKEN_EXEMPT_CMDS.has(finalCmd) && !isOwnerLevel(db, sender)) {
    if (!isGroupLicensed(db, from)) {
      const errorMp4Path = './error.mp4';
      const noLicenseCaption = `*⌞ ERROR (⁠・⁠o⁠・⁠) ⌝*\n\n(=⁠_⁠=) Este grupo no tiene una licencia activa kashira. Pídele un token a quien administra el bot y actívalo con *#token <código>*.`;
      if (fs.existsSync(errorMp4Path)) {
        await sock.sendMessage(from, {
          video: fs.readFileSync(errorMp4Path),
          mimetype: 'video/mp4',
          gifPlayback: true,
          caption: noLicenseCaption
        }, { quoted: msg });
      } else {
        await sendText(sock, from, noLicenseCaption, msg);
      }
      return;
    }
  }
  ...
  // ══════════════════════════════
    ...
    await sendText(sock, from, `[ (p^.^q) ] ¡Gracias por tu ${label} kashira! Se guardó como *#${id}*.${isGroup ? " Si resulta útil, este grupo puede ganar días extra de licencia." : ""}`, msg);
  }
  ...
  // ══════════════════════════════
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "ship.mp4"), [u1, u2]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "vs.mp4"), [u1, u2, ganador]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "mejor.mp4"), [u1, u2]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "rata.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "simp.mp4"), [target]);
  }
  ...
  // ══════════════════════════════
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "iq.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "gay.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "lesbian.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "bisexual.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "freaky.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "otaku.mp4"), [target]);
  }
    ...
    await animatePct(sock, from, msg, text, pct, path.join(__dirname, "funny.mp4"), [target]);
  }
  ...
  // ══════════════════════════════
    ...
    const text = list.map(r =>
      `*#${r.id}* [${r.tipo === "bug" ? "(×_×) Bug" : "(★ω★) Sugerencia"}]${r.rewarded ? " ✓ premiado" : ""} — @${r.senderNum} (${r.fecha})\nGrupo: ${r.groupJid || "DM (sin grupo)"}\n${r.texto}`
    ).join("\n──\n");
  ...
  else if (finalCmd === "rewardbug") {
    if (!isOwnerLevel(db, sender)) return sendText(sock, from, "[ x_x ] Solo el owner.", msg);

    const id = parseInt(args[1], 10);
    const days = parseInt(args[2], 10) || 2; // 2 días por defecto, como se anunció
    if (!id) return sendText(sock, from, "[ x_x ] Usa: #rewardbug <id> [días=2] kashira.\n\nRevisa los IDs con #bugs.", msg);

    db.bugReports = db.bugReports || [];
    const report = db.bugReports.find(r => r.id === id);
    if (!report) return sendText(sock, from, `[ x_x ] No encontré ningún reporte/sugerencia con el ID *#${id}* kashira.`, msg);
    if (!report.groupJid) return sendText(sock, from, `[ x_x ] El reporte *#${id}* no vino de un grupo (fue por DM), no hay a quién premiar kashira.`, msg);
    if (report.rewarded) return sendText(sock, from, `[ x_x ] El reporte *#${id}* ya fue premiado antes kashira.`, msg);

    const now = Date.now();
    db.groupLicenses = db.groupLicenses || {};
    const currentExpiry = db.groupLicenses[report.groupJid]?.expiresAt || 0;
    const base = Math.max(now, currentExpiry);
    const newExpiry = base + days * 24 * 60 * 60 * 1000;
    db.groupLicenses[report.groupJid] = { expiresAt: newExpiry, lastCode: `BUG#${id}`, activatedBy: senderNum, activatedAt: now };
    report.rewarded = true;
    saveDB(db);

    const expiryStr = new Date(newExpiry).toLocaleString("es-MX", { timeZone: "America/Mexico_City" });
    await sendText(sock, from, `[ (o^-')b ] Se premió el reporte *#${id}* con *${days}* día${days === 1 ? "" : "s"} de licencia para su grupo.\nNueva fecha de vencimiento: *${expiryStr}*`, msg);
  }
  ...
  // ══════════════════════════════
  //    SISTEMA DE LICENCIAS POR TOKEN
  // ══════════════════════════════
  else if (finalCmd === "gtoken") {
    if (!isOwner(sender)) return sendText(sock, from, "[ x_x ] Solo el owner original puede generar tokens kashira.", msg);

    const dias = parseInt(args[1], 10);
    const usos = parseInt(args[2], 10);
    if (!dias || dias <= 0 || !usos || usos <= 0) {
      return sendText(sock, from, "[ x_x ] Usa: #gtoken <días> <usos> kashira.\n\nEjemplo: #gtoken 30 5\n(un token que dura 30 días por grupo y se puede canjear en hasta 5 grupos distintos)", msg);
    }

    db.tokens = db.tokens || {};
    const code = generateTokenCode(db);
    db.tokens[code] = {
      durationDays: dias,
      maxUses: usos,
      usedCount: 0,
    ...
    await sendText(sock, from, `*⌞ Token Generado ⌝*\n\n(o^-')b Código: *${code}*\nDuración por grupo: *${dias} día${dias === 1 ? "" : "s"}*\nUsos disponibles: *${usos}*\n\n_Cada grupo que lo canjee con #token ${code} activa su propia licencia de ${dias} día${dias === 1 ? "" : "s"}, hasta agotar los ${usos} usos._`, msg);
  }

  else if (finalCmd === "token") {
    if (!isGroup) return sendText(sock, from, "[ x_x ] Usa este comando dentro del grupo que quieres activar kashira.", msg);

    const code = (args[1] || "").trim();
    if (!/^\d{6}$/.test(code)) {
      return sendText(sock, from, "[ x_x ] Usa: #token <código de 6 dígitos> kashira.", msg);
    }

    db.tokens = db.tokens || {};
    const t = db.tokens[code];
    if (!t) return sendText(sock, from, "[ x_x ] Ese token no existe kashira.", msg);
    if (t.usedCount >= t.maxUses) return sendText(sock, from, "[ x_x ] Ese token ya agotó todos sus usos kashira.", msg);

    const now = Date.now();
    db.groupLicenses = db.groupLicenses || {};
    const currentExpiry = db.groupLicenses[from]?.expiresAt || 0;
    const base = Math.max(now, currentExpiry); // si ya tenía licencia activa, se extiende en vez de reiniciar
    const newExpiry = base + t.durationDays * 24 * 60 * 60 * 1000;

    t.usedCount++;
    t.redemptions.push({ group: from, redeemedBy: senderNum, redeemedAt: now });
    db.groupLicenses[from] = { expiresAt: newExpiry, lastCode: code, activatedBy: senderNum, activatedAt: now };
    saveDB(db);

    const expiryStr = new Date(newExpiry).toLocaleString("es-MX", { timeZone: "America/Mexico_City" });
    await sendText(sock, from, `*⌞ Token Activado ⌝*\n\n(≧▽≦) ¡Listo kashira! Este grupo ya tiene licencia activa.\n\nVence: *${expiryStr}*\nUsos restantes de este token: *${t.maxUses - t.usedCount}*`, msg);
  }

  else if (finalCmd === "tokens") {
    if (!isOwnerLevel(db, sender)) return sendText(sock, from, "[ x_x ] Solo el owner.", msg);

    db.tokens = db.tokens || {};
    const codes = Object.keys(db.tokens);
    if (!codes.length) return sendText(sock, from, "[ (._.) ] No hay tokens generados kashira.", msg);

    const lines = codes.map(code => {
      const t = db.tokens[code];
      return `*${code}* — ${t.durationDays}d, usos: ${t.usedCount}/${t.maxUses}`;
    });
    await sendText(sock, from, `*⌞ Tokens Generados ⌝*\n\n${lines.join("\n")}`, msg);
  }

  else if (finalCmd === "licencia") {
    if (!isGroup) return sendText(sock, from, "[ x_x ] Este comando es para usarse dentro de un grupo kashira.", msg);

    const lic = db.groupLicenses?.[from];
    if (!lic || lic.expiresAt <= Date.now()) {
      return sendText(sock, from, "[ x_x ] Este grupo no tiene una licencia activa kashira. Pide un token y actívalo con #token <código>.", msg);
    }
    const expiryStr = new Date(lic.expiresAt).toLocaleString("es-MX", { timeZone: "America/Mexico_City" });
    await sendText(sock, from, `*⌞ Licencia del Grupo ⌝*\n\n(o^-')b Licencia activa kashira.\nVence: *${expiryStr}*`, msg);
  }

  else if (finalCmd === "trial") {
    if (!isGroup) return sendText(sock, from, "[ x_x ] La prueba gratis solo se puede activar dentro de un grupo kashira.", msg);

    db.trialUsedGroups = db.trialUsedGroups || {};
    if (hasUsedTrial(db, from)) {
      return sendText(sock, from, "[ (¬_¬) ] Este grupo ya usó su prueba gratis antes kashira (aunque me hayan sacado y vuelto a meter, solo se puede una vez). Pide un token con #token <código>.", msg);
    }
    ...
    const currentExpiry = db.groupLicenses[from]?.expiresAt || 0;
    const base = Math.max(now, currentExpiry);
    const newExpiry = base + TRIAL_DAYS * 24 * 60 * 60 * 1000;

    db.groupLicenses[from] = { expiresAt: newExpiry, lastCode: "TRIAL", activatedBy: senderNum, activatedAt: now };
    db.trialUsedGroups[from] = true;
    saveDB(db);

    const expiryStr = new Date(newExpiry).toLocaleString("es-MX", { timeZone: "America/Mexico_City" });
    await sendText(sock, from, `*⌞ Prueba Gratis Activada ⌝*\n\n(≧▽≦) ¡${TRIAL_DAYS} días de prueba activados kashira!\n\nVence: *${expiryStr}*\n\n_Cuando se acabe, pide un token al owner y actívalo con #token <código>._`, msg);
  }

  else if (finalCmd === "licenseall") {
    if (!isOwner(sender)) return sendText(sock, from, "[ x_x ] Solo el owner original puede usar este comando kashira.", msg);

    // Uso: #licenseall dd/mm/yyyy [HH:mm]   (hora opcional, default 12:00, zona America/Mexico_City)
    const dateArg = args[1];
    const timeArg = args[2] || "12:00";
    const dateMatch = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(dateArg || "");
    const timeMatch = /^(\d{1,2}):(\d{2})$/.exec(timeArg);
    if (!dateMatch || !timeMatch) {
      return sendText(sock, from, "[ x_x ] Usa: #licenseall dd/mm/yyyy [HH:mm] kashira.\n\nEjemplo: #licenseall 20/07/2026 12:00", msg);
    }

    const [, dd, mm, yyyy] = dateMatch;
    const [, HH, MinM] = timeMatch;
    // Se arma como ISO con offset fijo de Ciudad de México (-06:00, sin horario de verano)
    // para que la fecha/hora sea exacta sin importar en qué zona horaria corra el servidor.
    const iso = `${yyyy}-${mm.padStart(2, "0")}-${dd.padStart(2, "0")}T${HH.padStart(2, "0")}:${MinM}:00-06:00`;
    const expiresAt = new Date(iso).getTime();
    if (isNaN(expiresAt)) {
      return sendText(sock, from, "[ x_x ] Esa fecha/hora no es válida kashira.", msg);
    }
    ...
    try {
      ...
      return sendText(sock, from, "[ x_x ] No pude obtener la lista de grupos: " + e.message, msg);
    }

    const groupIds = Object.keys(allGroups || {});
    if (!groupIds.length) return sendText(sock, from, "[ (._.) ] No estoy en ningún grupo kashira.", msg);

    const now = Date.now();
    db.groupLicenses = db.groupLicenses || {};
    for (const gid of groupIds) {
      db.groupLicenses[gid] = { expiresAt, lastCode: "MASSGRANT", activatedBy: senderNum, activatedAt: now };
    }
    saveDB(db);

    const expiryStr = new Date(expiresAt).toLocaleString("es-MX", { timeZone: "America/Mexico_City" });
    await sendText(sock, from, `*⌞ Licencia Masiva Aplicada ⌝*\n\n(o^-')b Se activó licencia en *${groupIds.length}* grupo${groupIds.length === 1 ? "" : "s"} kashira.\nVence: *${expiryStr}*`, msg);
  }
  ...
  // ══════════════════════════════
    ...
    const errorMp4Path = './error.mp4'; // Cambiamos a .mp4
...
(o˘◡˘o) *¡Este grupo tiene 2 días de prueba gratis!* Actívalos con *#trial* (solo funciona una vez por grupo, aunque me saquen y me vuelvan a meter no se puede repetir). Cuando se acabe, pide un token a mi owner y actívalo con *#token <código>*.
...
async function startBot() {
  ...
  sock.ev.on("group-participants.update", async (update) => {
    try {
      ...
      // (el flag de la prueba gratis #trial NO se toca aquí, ese sigue siendo
      // permanente para siempre sin importar si el bot sale y vuelve).
